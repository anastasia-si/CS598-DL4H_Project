...
if __name__ == '__main__':
    ...
    admissions = pd.read_csv(hp.mimic_dir + 'ADMISSIONS.csv', usecols=dtype.keys(), dtype=dtype,
    ...
    diag_proc = diag_proc.apply(
        lambda x: x.mask(x.map(x.value_counts()) < hp.min_count, 'other') if x.name in ['ICD9_CODE'] else x)
