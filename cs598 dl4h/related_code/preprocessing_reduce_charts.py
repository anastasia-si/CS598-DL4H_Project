...
fraction_inspired_oxygen = [189, 190, 727, 1040, 1206, 1863, 2518, 2981, 3420, 3422, 7018, 7041, 7570, 223835, 226754,
                            227009, 227010]
respiratory_rate = [614, 615, 618, 619, 651, 653, 1884, 3603, 6749, 7884, 8113, 220210, 224422, 224688, 224689, 224690,
                    226774, 227050]
...
if __name__ == '__main__':
    ...
    # Not parsing dates
    for df in tqdm(
            pd.read_csv(hp.mimic_dir + 'CHARTEVENTS.csv', usecols=dtype.keys(), dtype=dtype, chunksize=chunksize)):
        df = df[df['ICUSTAY_ID'].notna() & df['VALUENUM'].notna() & (df['ITEMID'].isin(relevant_ids)) & (
                    df['VALUENUM'] > 0)]
        # convert units
        ...
        df = df[~(df['ITEMID'].isin(diastolic_blood_pressure + systolic_blood_pressure + mean_blood_pressure) & (
                    df.VALUENUM > 250))]
